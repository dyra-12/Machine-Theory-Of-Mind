"""
Experiment runner for Week 1 - integrates with existing agents
"""
import argparse
import json
...
class Week1ExperimentRunner:
    """Runs Week 1 experiments with existing components"""
    ...
    def run_single_episode(self, lambda_social: float) -> Dict[str, float]:
        """
        Run a single negotiation episode using simple policies
        """
        state = self.env.reset()
        mental_state = self.observer.create_initial_mental_state()
        
        # Simple agent policy for Week 1 testing
        while not state.is_terminal():
            # Simple policy based on lambda
            if lambda_social > 1.0:
                offer = (3, 7)  # Social-focused
            elif lambda_social > 0.5:
                offer = (5, 5)  # Balanced
            else:
                offer = (7, 3)  # Task-focused
                
            state = self.env.step(state, offer)
            
            # Update mental state based on action
            ...
            mental_state.competence = np.clip(mental_state.competence + 0.3 * c_delta, 0.0, 1.0)
    ...
    def run_experiment(self) -> List[Dict[str, Any]]:
        """Run full experiment across lambda values"""
        results = []
        ...
        for lambda_val in lambda_values:
            for run in range(num_runs):
                ...
                print(f"λ={lambda_val:.1f}, Run {run+1}: "
