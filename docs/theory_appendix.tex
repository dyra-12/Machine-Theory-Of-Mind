...
\paragraph{Experimental link.} Posterior trajectories already logged by \texttt{experiments/run\_week4.py} and archived in \texttt{docs/internal/results/week4/analysis\_report.md} can be extended with log-likelihood ratios to visualise the exponential decay of discarded hypotheses.
...
\paragraph{Sketch.} The SIQ expectation is smooth and strictly increasing in each component, so convex multi-objective results apply. In practice, $\lambda$ coincides with the \texttt{lambda\_social} knob exposed by \texttt{src/agents/bayesian\_mtom\_agent.py}. Sweeping $\lambda$ via \texttt{experiments/run\_trace\_sweep\_extended.py} plus SIQ logging in \texttt{demo/trace\_dashboard.py} traces the empirical frontier.
