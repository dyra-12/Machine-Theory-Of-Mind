...
The study followed a repeated-measures comparative design, where participants evaluated short interaction transcripts generated by two agent types:
...
These dimensions are well-established in the social cognition literature and align with the latent variables modeled by the MToM framework. The same scale orientation was used across all dimensions to reduce cognitive load.
...
**Storage:** Local secure storage; only anonymized and aggregated data are included in the public repository (`data/human_pilot/`)
